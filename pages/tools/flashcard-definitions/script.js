...
const uiStrings = {
    ...
    footerCredits: {tr: "Taygun Kara tarafından geliştirilmiştir.", en: "Developed by Taygun Kara."} // Bu artık doğrudan HTML'de olacak.
};
...
let showAllCardsBtn, showProgressReportBtn, homeBtn, toolsBtn; // homeBtn ve toolsBtn eklendi
...
function getDOMElements() {
    ...
    homeBtn = document.getElementById('home-btn'); // Eklendi
    toolsBtn = document.getElementById('tools-btn'); // Eklendi
...
function updateUIStrings() {
    ...
    document.querySelectorAll('[data-translate]').forEach(element => {
        ...
        if (uiStrings[key] && uiStrings[key][lang] !== undefined) {
            ...
            // Fallback to default language if current language string is missing
    ...
    if (progressHeaderRow && uiStrings.tableColTerm[lang] && uiStrings.tableColDifficulty[lang] && uiStrings.tableColLastReview[lang] && uiStrings.tableColNextReview[lang] && uiStrings.tableColCorrect[lang] && uiStrings.tableColIncorrect[lang]) {
...
document.addEventListener('DOMContentLoaded', () => { 
    getDOMElements(); 
    state.currentLanguage = localStorage.getItem('flashcardAppLanguageV2') || config.defaultLang; 
    ...
    if(goToWelcomeBtn) goToWelcomeBtn.addEventListener('click', goToWelcomeScreen); 
    if (exitLearningBtn) exitLearningBtn.addEventListener('click', goToWelcomeScreen); 
    ...
    if (languageToggle) languageToggle.addEventListener('click', handleLanguageChange); 
    if(showAllCardsBtn) showAllCardsBtn.addEventListener('click', () => { populateAllCardsTable(); showReportModal(allCardsReportContainer); }); 
    if(showProgressReportBtn) showProgressReportBtn.addEventListener('click', () => { populateProgressReportTable(); showReportModal(progressReportContainer); }); 
    ...
    // Ana navigasyon butonları başlangıçta görünür olmalı
    if(homeBtn) homeBtn.classList.remove('hidden');
    if(toolsBtn) toolsBtn.classList.remove('hidden');
